...
1. 文件开头`$$`后换行，不然识别器不准导致无法编译
2. 编号`1.`后去掉空格，不然预处理会在其前面加一行，导致编译失败
3. 个别复杂情形也会导致识别错乱，例`>` `=`等
...
创建了自定义样式文件 assets/_custom.scss
使用 KaTeX 推荐的方法 overflow: auto hidden，只显示横向滚动条，避免垂直滚动条
