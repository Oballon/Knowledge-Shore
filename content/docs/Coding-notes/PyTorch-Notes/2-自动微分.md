...
自动微分基于**链式法则（Chain Rule）**，复合函数 $y = f(g(x))$ 的导数为：
...
3. **反向传播**：调用 `backward()` 时，从根到叶遍历图，按链式法则累积梯度，记入`.grad`中
...
ReLU、sqrt 等在部分点不可微。PyTorch 采用约定：可微则用导数；凸函数用最小范数次梯度；未定义处可能返回 NaN。详见 [Autograd 机制](https://pytorch.org/docs/stable/notes/autograd.html)。
