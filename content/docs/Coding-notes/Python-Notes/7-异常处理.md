...
Python 中的异常都是类，抛出异常就是创建该异常类的实例

自定义异常类可以更好地表达特定的错误情况，提高代码可读性和错误处理的精确性。
...
class FileProcessingError(Exception):
    ...
    def __str__(self):
        """自定义异常显示"""
        base_msg = f"{self.message} (文件: {self.filename}"
        if self.line_number:
...
# 使用
try:
    raise FileProcessingError("格式错误", "data.txt", line_number=42)
except FileProcessingError as e:
    print(e)  # 格式错误 (文件: data.txt, 行号: 42)
```

#### 使用规则

1. **继承 Exception**：自定义异常必须继承 `Exception` 或其子类
2. **命名规范**：异常类名以 `Error` 或 `Exception` 结尾
3. **文档字符串**：使用文档字符串说明异常用途
4. **初始化方法**：重写 `__init__` 添加自定义属性，调用 `super().__init__()`
5. **异常消息**：通过 `super().__init__(message)` 设置异常消息
6. **继承层次**：可以创建异常基类和具体异常类，形成异常层次结构
7. **自定义显示**：重写 `__str__` 方法自定义异常显示格式
